...
def register():
    ...
    # 4:39. Сейчас я считаю это лучшей фильтрацией полей
    # по пересечению множеств полей модели и полей JSON
    data = {key: raw_data[key] for key in set(raw_data.keys()) &
            set(get_fields_names(User))}
...
def get_or_change_user(user_id):
    ...
    if request.method == "GET":
        ...
        response["documents"] = [document.to_dict() for document
                                 in user_documents]
        return response
...
def create_document():
    ...
    document = Document(id=document_id, type_id=document_type,
                        data=str(raw_data))
...
def get_or_change_document(document_id):
    ...
    if request.method == "PUT":
        ...
        detail_data = json.loads(
            document.data.replace("'", '"').replace("None", "null"))
        print(detail_data)
        ...
        for data_second_step in data_first_step["Data"]:
            for data_third_step in data_second_step["Users"]:
                ...
                for document in documents_data:
                    ...
                    new_document = Document(id=document_id,
